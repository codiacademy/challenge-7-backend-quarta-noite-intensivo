...
export class SaleRepository {
  create(data: CreateSaleInput & { totalPrice:number; date:Date }) { return prisma.sale.create({ data }); }
  findAll(filter?: any) {
    const where:any = {};
  ...
  update(id:number, data: Partial<UpdateSaleInput & { totalPrice?:number; date?:Date }>) { return prisma.sale.update({ where:{ id }, data }); }
  delete(id:number) { return prisma.sale.delete({ where:{ id } }); }
}
